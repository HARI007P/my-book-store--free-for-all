...
function Signup() {
  ...
  const resendOTP = async () => {
    if (!userData.email || !userData.fullname) { toast.error("Please enter your details again"); setStep(1); return; }
    try {
      setLoading(true);
        ...
        {step === 2 && (
          <form onSubmit={handleSubmit(handleOTPSubmit)} className="space-y-6">
            <div>
              ...
              <input type="text" placeholder="OTP" {...register("otp", { required: true })}
                className="w-full px-4 py-2 bg-white/20 text-white rounded-xl placeholder-white/70 focus:outline-none" />
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => setStep(1)}
                className="w-1/2 py-3 rounded-xl bg-gray-500 hover:bg-gray-600 text-white transition duration-300">Back</button>
              <button type="submit" disabled={loading}
                className={`w-1/2 py-3 rounded-xl transition duration-300 ${loading ? "bg-blue-400" : "bg-blue-500 hover:bg-blue-600"} text-white`}>
            ...
            <p className="text-sm text-white/80 mt-2 text-center">
              Didn't receive OTP?{" "}
              <button type="button" onClick={resendOTP} disabled={loading} className="text-blue-200 hover:underline">Resend OTP</button>
            </p>
          </form>
        )}
      </div>
